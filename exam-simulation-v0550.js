...
import {createApiClient,ApiError} from './api-v040.js?v=0.5.50';
...
function answerValue(){
 ...
 if(question.type==='text')return document.querySelector('#exam-text-answer')?.value.trim()||'';
...
async function persistCurrent(){
 ...
 if(saveState)saveState.textContent='Tersimpan ✓';
...
function renderExam(){
 ...
 const body=el('div',{class:'exam-question-card'},[
  el('div',{class:'exam-question-head'},[
   el('div',{class:'exam-tags'},[text('span','ENGLISH'),text('span',question.difficulty||'Grade 2')]),
 ...
 if(question.type==='text'){
  const input=el('textarea',{id:'exam-text-answer',rows:'3',maxlength:'500',placeholder:'Write your answer here…'});
 ...
 const saveState=text('span',activeExam.savedAnswer?'Tersimpan ✓':'Jawaban belum disimpan','exam-save-state');saveState.id='exam-save-state';
...
function renderExamResult(result){
 ...
 card.append(
  ...
  text('div',summary.score>=80?'🏆':summary.score>=60?'🚀':'🌱','exam-result-emoji'),
  text('h1',result.title),
  el('div',{class:'exam-result-score'},[text('strong',String(summary.score)),text('span','/100')]),
  el('div',{class:'exam-result-stats'},[
   text('span',`✅ ${summary.correct} correct`),
   text('span',`❌ ${summary.wrong} wrong`),
   text('span',`⬜ ${summary.unanswered} unanswered`),
  ]),
  text('p','Nilai baru dibuka setelah seluruh simulasi selesai, seperti ujian sungguhan.','exam-result-note'),
 );

 const review=el('section',{class:'exam-review'});review.append(text('h2','Review Answers'));
 for(const item of result.results){
  const detail=el('details',{class:`exam-review-item ${item.correct?'is-correct':'is-wrong'}`});
  detail.append(
   el('summary',{},[text('span',`Question ${item.position}`),text('strong',item.correct?'✓ Correct':item.answer?'✕ Check again':'— Unanswered')]),
   el('div',{class:'exam-review-body'},[
    item.prompt?text('p',item.prompt,'exam-review-prompt'):null,
    text('p',`Your answer: ${item.answer||'—'}`),
    text('p',`Correct answer: ${item.correctAnswer||'—'}`),
    item.explanation?text('p',item.explanation,'exam-review-explanation'):null,
   ]),
