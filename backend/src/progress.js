...
function levelForXp(xp) {
  ...
  return {
    ...
    progressPercent: next ? Math.max(0, Math.min(100, Math.round(((xp - current.minXp) / (next.minXp - current.minXp)) * 100))) : 100,
  };
}
...
function streakInfo(completedAtValues) {
  const unique = [...new Set(completedAtValues.filter(Boolean).map(jakartaDateKey))].map(dayNumber).sort((a, b) => a - b);
  if (!unique.length) return { current: 0, longest: 0 };
  let longest = 1;
  let run = 1;
  for (let i = 1; i < unique.length; i += 1) {
    if (unique[i] === unique[i - 1] + 1) run += 1; else run = 1;
  ...
  for (let i = unique.length - 1; i > 0; i -= 1) {
    if (unique[i - 1] === unique[i] - 1) current += 1; else break;
  }
...
export async function progressForFamily(env, familyId, profileId = null) {
  ...
  return (result.results || []).map((row) => ({ profileId: row.profileId, subjectId: row.subjectId, attempted: Number(row.attempted), correct: Number(row.correct), lastPracticedAt: Number(row.lastPracticedAt) }));
}
